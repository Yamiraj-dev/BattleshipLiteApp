...
namespace BattleshipLiteLibrary
{
    public static class GameLogic
    {
        ...
        {
            bool output = false;
            (string row, int column) = SpotIdentifier(location);
        ...
        public static (string row, int column) SpotIdentifier(string shot)
        {
            ...
            if (shot.Length != 2)
            {
                throw new ArgumentOutOfRangeException(shot, "This was an invalid shot type.");
            }

            char[] shotArray = shot.ToArray(); // char is one character

            row = shotArray[0].ToString();
            column = int.Parse(shotArray[1].ToString()); // we cannot return the char directly as characters have numbers assigned to them, we take the integer parsed directly from the string entered.
